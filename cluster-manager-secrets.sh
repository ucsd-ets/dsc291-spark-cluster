...
if [ "$#" -eq 1 ] && [ $1 == 'create' ]; then
	...
	NUM_WORKERS=2
        CLUSTER_UID=$(id -u)
        SECRET=$(tr -dc A-Za-z0-9 </dev/urandom | head -c 20)
	cat $BASEDIR/spark-cluster-secrets.yaml.template | sed -e "s/\$JUPYTER_TOKEN/$JUPYTER_TOKEN/" -e "s/\$NUM_WORKERS/$NUM_WORKERS/" -e "s/\$USERNAME/$USER/g" -e "s/\$USER_ID/$CLUSTER_UID/g" -e "s/\$SECRET/$SECRET/g" > $BASEDIR/spark-cluster.yaml
	...
	echo "=> Next create a SSH tunnel from your personal computer using the following command:"
        echo "        $SSH_COMMAND"
	echo ""
        echo "=> Spark secret, pass to your group = $SECRET"
